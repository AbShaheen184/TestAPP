...
public class ButtonBarLayout extends LinearLayout {
    ...
    public ButtonBarLayout(Context context, AttributeSet attributeSet) {
        ...
        androidx.core.view.s0.l(this, context, iArr, attributeSet, typedArrayObtainStyledAttributes, 0);
    ...
    public final void onMeasure(int i, int i2) {
        ...
        int paddingBottom = 0;
        if (this.e) {
            if (size > this.z && this.y) {
                setStacked(false);
            }
            this.z = size;
        }
        if (this.y || View.MeasureSpec.getMode(i) != 1073741824) {
            iMakeMeasureSpec = i;
            z = false;
        } else {
        ...
        if (this.e && !this.y && (getMeasuredWidthAndState() & (-16777216)) == 16777216) {
            setStacked(true);
            z = true;
        }
        if (z) {
