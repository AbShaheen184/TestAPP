package androidx.compose.runtime;

/* JADX INFO: compiled from: r8-map-id-e67002de5a40c891c754bff9e68f2bf8edbc7d2191d59a60e5e311fc3b432df9 */
/* JADX INFO: loaded from: classes.dex */
public final class g0 extends androidx.compose.runtime.snapshots.x implements w2 {
    public f0 A = new f0(androidx.compose.runtime.snapshots.n.j().g());
    public final kotlin.jvm.functions.a y;
    public final f z;

    public g0(f fVar, kotlin.jvm.functions.a aVar) {
        this.y = aVar;
        this.z = fVar;
    }

    @Override // androidx.compose.runtime.snapshots.w
    public final void b(androidx.compose.runtime.snapshots.y yVar) {
        yVar.getClass();
        this.A = (f0) yVar;
    }

    @Override // androidx.compose.runtime.snapshots.w
    public final androidx.compose.runtime.snapshots.y c() {
        return this.A;
    }

    /* JADX WARN: Code duplicated, block: B:101:0x00a3 A[EDGE_INSN: B:101:0x00a3->B:31:0x00a3 BREAK  A[LOOP:1: B:16:0x0049->B:30:0x009e], SYNTHETIC] */
    /* JADX WARN: Code duplicated, block: B:29:0x009c A[DONT_INVERT] */
    /* JADX WARN: Code duplicated, block: B:30:0x009e A[Catch: all -> 0x0038, LOOP:1: B:16:0x0049->B:30:0x009e, LOOP_END, TryCatch #1 {all -> 0x0038, blocks: (B:8:0x0023, B:10:0x002f, B:13:0x003b, B:16:0x0049, B:18:0x005c, B:20:0x0068, B:22:0x0072, B:24:0x008a, B:26:0x0090, B:30:0x009e, B:31:0x00a3), top: B:93:0x0023 }] */
    public final f0 g(f0 f0Var, androidx.compose.runtime.snapshots.f fVar, boolean z, kotlin.jvm.functions.a aVar) {
        f0 f0Var2;
        f fVar2;
        int i;
        if (f0Var.c(this, fVar)) {
            if (z) {
                androidx.compose.runtime.collection.b bVarI = t.i();
                Object[] objArr = bVarI.e;
                int i2 = bVarI.z;
                for (int i3 = 0; i3 < i2; i3++) {
                    ((r) objArr[i3]).b();
                }
                try {
                    androidx.collection.f0 f0Var3 = f0Var.e;
                    androidx.appcompat.widget.c2 c2Var = p2.a;
                    androidx.compose.runtime.internal.g gVar = (androidx.compose.runtime.internal.g) c2Var.u();
                    if (gVar == null) {
                        gVar = new androidx.compose.runtime.internal.g();
                        c2Var.X(gVar);
                    }
                    int i4 = gVar.a;
                    Object[] objArr2 = f0Var3.b;
                    int[] iArr = f0Var3.c;
                    long[] jArr = f0Var3.a;
                    int length = jArr.length - 2;
                    if (length >= 0) {
                        int i5 = 0;
                        while (true) {
                            long j = jArr[i5];
                            if ((((~j) << 7) & j & (-9187201950435737472L)) == -9187201950435737472L) {
                                if (i5 != length) {
                                    break;
                                    break;
                                }
                                i5++;
                            } else {
                                int i6 = 8;
                                int i7 = 8 - ((~(i5 - length)) >>> 31);
                                int i8 = 0;
                                while (i8 < i7) {
                                    if ((j & 255) < 128) {
                                        int i9 = (i5 << 3) + i8;
                                        i = i6;
                                        androidx.compose.runtime.snapshots.w wVar = (androidx.compose.runtime.snapshots.w) objArr2[i9];
                                        gVar.a = i4 + iArr[i9];
                                        kotlin.jvm.functions.l lVarE = fVar.e();
                                        if (lVarE != null) {
                                            lVarE.invoke(wVar);
                                        }
                                    } else {
                                        i = i6;
                                    }
                                    j >>= i;
                                    i8++;
                                    i6 = i;
                                }
                                if (i7 != i6) {
                                    break;
                                }
                                if (i5 != length) {
                                    break;
                                }
                                i5++;
                            }
                        }
                    }
                    gVar.a = i4;
                } finally {
                    Object[] objArr3 = bVarI.e;
                    int i10 = bVarI.z;
                    for (int i11 = 0; i11 < i10; i11++) {
                        ((r) objArr3[i11]).a();
                    }
                }
            }
            return f0Var;
        }
        androidx.collection.f0 f0Var4 = new androidx.collection.f0();
        androidx.appcompat.widget.c2 c2Var2 = p2.a;
        androidx.compose.runtime.internal.g gVar2 = (androidx.compose.runtime.internal.g) c2Var2.u();
        if (gVar2 == null) {
            gVar2 = new androidx.compose.runtime.internal.g();
            c2Var2.X(gVar2);
        }
        int i12 = gVar2.a;
        androidx.compose.runtime.collection.b bVarI2 = t.i();
        Object[] objArr4 = bVarI2.e;
        int i13 = bVarI2.z;
        for (int i14 = 0; i14 < i13; i14++) {
            ((r) objArr4[i14]).b();
        }
        try {
            gVar2.a = i12 + 1;
            Object objJ = androidx.compose.runtime.snapshots.r.j(new androidx.compose.foundation.layout.e1(this, gVar2, f0Var4, i12, 3), aVar);
            gVar2.a = i12;
            Object[] objArr5 = bVarI2.e;
            int i15 = bVarI2.z;
            for (int i16 = 0; i16 < i15; i16++) {
                ((r) objArr5[i16]).a();
            }
            Object obj = androidx.compose.runtime.snapshots.n.c;
            synchronized (obj) {
                try {
                    androidx.compose.runtime.snapshots.f fVarJ = androidx.compose.runtime.snapshots.n.j();
                    Object obj2 = f0Var.f;
                    if (obj2 == f0.h || (fVar2 = this.z) == null || !fVar2.b(objJ, obj2)) {
                        f0 f0Var5 = this.A;
                        synchronized (obj) {
                            androidx.compose.runtime.snapshots.y yVarM = androidx.compose.runtime.snapshots.n.m(f0Var5, this);
                            yVarM.a(f0Var5);
                            yVarM.a = fVarJ.g();
                            f0Var2 = (f0) yVarM;
                            f0Var2.e = f0Var4;
                            f0Var2.g = f0Var2.d(this, fVarJ);
                            f0Var2.f = objJ;
                        }
                        return f0Var2;
                    }
                    f0Var.e = f0Var4;
                    f0Var.g = f0Var.d(this, fVarJ);
                    f0Var2 = f0Var;
                } catch (Throwable th) {
                    throw th;
                }
            }
            androidx.compose.runtime.internal.g gVar3 = (androidx.compose.runtime.internal.g) p2.a.u();
            if (gVar3 == null || gVar3.a != 0) {
                return f0Var2;
            }
            androidx.compose.runtime.snapshots.n.j().m();
            synchronized (obj) {
                androidx.compose.runtime.snapshots.f fVarJ2 = androidx.compose.runtime.snapshots.n.j();
                f0Var2.c = fVarJ2.g();
                f0Var2.d = fVarJ2.h();
                return f0Var2;
            }
        } catch (Throwable th2) {
            Object[] objArr6 = bVarI2.e;
            int i17 = bVarI2.z;
            for (int i18 = 0; i18 < i17; i18++) {
                ((r) objArr6[i18]).a();
            }
            throw th2;
        }
    }

    @Override // androidx.compose.runtime.w2
    public final Object getValue() {
        kotlin.jvm.functions.l lVarE = androidx.compose.runtime.snapshots.n.j().e();
        if (lVarE != null) {
            lVarE.invoke(this);
        }
        androidx.compose.runtime.snapshots.f fVarJ = androidx.compose.runtime.snapshots.n.j();
        return g((f0) androidx.compose.runtime.snapshots.n.i(this.A, fVarJ), fVarJ, true, this.y).f;
    }

    public final f0 h() {
        androidx.compose.runtime.snapshots.f fVarJ = androidx.compose.runtime.snapshots.n.j();
        return g((f0) androidx.compose.runtime.snapshots.n.i(this.A, fVarJ), fVarJ, false, this.y);
    }

    public final String toString() {
        StringBuilder sb = new StringBuilder("DerivedState(value=");
        f0 f0Var = (f0) androidx.compose.runtime.snapshots.n.h(this.A);
        sb.append(f0Var.c(this, androidx.compose.runtime.snapshots.n.j()) ? String.valueOf(f0Var.f) : "<Not calculated>");
        sb.append(")@");
        sb.append(hashCode());
        return sb.toString();
    }
}
