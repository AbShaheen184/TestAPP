...
public final class f0 {
    ...
    public static final void c(float[] fArr, androidx.compose.ui.geometry.a aVar) {
        if (fArr.length < 16) {
            return;
        }
        float f = fArr[0];
        ...
        aVar.b = Math.min(f19, Math.min(f26, Math.min(f32, f37)));
        aVar.c = Math.min(f22, Math.min(f28, Math.min(f34, f38)));
        aVar.d = Math.max(f19, Math.max(f26, Math.max(f32, f37)));
        aVar.e = Math.max(f22, Math.max(f28, Math.max(f34, f38)));
    }
    ...
    public final boolean equals(Object obj) {
        if (obj instanceof f0) {
            return kotlin.jvm.internal.l.a(this.a, ((f0) obj).a);
        }
        return false;
    }
    ...
    public final String toString() {
        ...
        sb.append(fArr[15]);
        sb.append("|\n        ");
        return kotlin.text.l.j(sb.toString());
    }
}
