...
public final class DebugProbesImpl {
    ...
    static {
        ...
        ARTIFICIAL_FRAME = _COROUTINE.a.d(new Exception(), b.class.getSimpleName());
    ...
    private final void build(Job job, Map<Job, DebugCoroutineInfoImpl> map, StringBuilder sb, String str) {
        ...
        if (debugCoroutineInfoImpl != null) {
            StackTraceElement stackTraceElement = (StackTraceElement) p.D(debugCoroutineInfoImpl.lastObservedStackTrace$kotlinx_coroutines_core());
            ...
            b0.i(sbP, getDebugString(job), ", continuation is ", state$kotlinx_coroutines_core, " at line ");
            sbP.append(stackTraceElement);
            sbP.append('\n');
            sb.append(sbP.toString());
            str = str + '\t';
        } else if (!(job instanceof ScopeCoroutine)) {
            StringBuilder sbP2 = androidx.compose.runtime.j.p(str);
            sbP2.append(getDebugString(job));
            sbP2.append('\n');
    ...
    private final <R> List<R> dumpCoroutinesInfoImpl(final kotlin.jvm.functions.p pVar) {
        if (isInstalled$kotlinx_coroutines_debug()) {
            return k.O(k.N(new h(3, p.u(getCapturedCoroutines()), new DebugProbesImpl$dumpCoroutinesInfoImpl$$inlined$sortedBy$1()), new l() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl.dumpCoroutinesInfoImpl.3
        ...
        kotlinx.coroutines.future.a.u("Debug probes are not installed");
        return null;
    }
    ...
    private final void dumpCoroutinesSynchronized(PrintStream printStream) {
        ...
        g gVar = new g(p.u(getCapturedCoroutines()), true, new q0(15));
        Comparator comparator = new Comparator() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesSynchronized$$inlined$sortedBy$1
            ...
            public final int compare(T t, T t2) {
                return kotlin.comparisons.a.d(Long.valueOf(((DebugProbesImpl.CoroutineOwner) t).info.sequenceNumber), Long.valueOf(((DebugProbesImpl.CoroutineOwner) t2).info.sequenceNumber));
            }
        };
    ...
    private final int findIndexOfFrame(int i, StackTraceElement[] stackTraceElementArr, List<StackTraceElement> list) {
        StackTraceElement stackTraceElement = (StackTraceElement) o.v(i, stackTraceElementArr);
    ...
    private final boolean isInternalMethod(StackTraceElement stackTraceElement) {
        return r.u(stackTraceElement.getClassName(), "kotlinx.coroutines", false);
    }
    ...
    private final void startWeakRefCleanerThread() {
        f fVar = new f(new com.app.mlounge.ui.theme.f(4));
        fVar.setDaemon(true);
    ...
    private final void updateState(kotlin.coroutines.d<?> dVar, String str) {
        if (isInstalled$kotlinx_coroutines_debug()) {
            if (ignoreCoroutinesWithEmptyContext && dVar.getContext() == kotlin.coroutines.k.e) {
                return;
            }
    ...
    public final List<DebugCoroutineInfo> dumpCoroutinesInfo() {
        if (isInstalled$kotlinx_coroutines_debug()) {
            return k.O(k.N(new h(3, p.u(getCapturedCoroutines()), new DebugProbesImpl$dumpCoroutinesInfoImpl$$inlined$sortedBy$1()), new l() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesInfo$$inlined$dumpCoroutinesInfoImpl$1
                @Override // kotlin.jvm.functions.l
    ...
    public final Object[] dumpCoroutinesInfoAsJsonAndReferences() {
        ...
        for (DebugCoroutineInfo debugCoroutineInfo : listDumpCoroutinesInfo) {
            ...
            sbX.append("\"\n                } \n                ");
            arrayList3.add(kotlin.text.l.j(sbX.toString()));
            arrayList2.add(debugCoroutineInfo.getLastObservedFrame());
            arrayList.add(debugCoroutineInfo.getLastObservedThread());
        }
        return new Object[]{androidx.compose.runtime.j.l(new StringBuilder("["), p.I(arrayList3, null, null, null, null, 63), ']'), arrayList.toArray(new Thread[0]), arrayList2.toArray(new d[0]), listDumpCoroutinesInfo.toArray(new DebugCoroutineInfo[0])};
    }

    public final List<DebuggerInfo> dumpDebuggerInfo() {
        if (isInstalled$kotlinx_coroutines_debug()) {
            return k.O(k.N(new h(3, p.u(getCapturedCoroutines()), new DebugProbesImpl$dumpCoroutinesInfoImpl$$inlined$sortedBy$1()), new l() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpDebuggerInfo$$inlined$dumpCoroutinesInfoImpl$1
                @Override // kotlin.jvm.functions.l
    ...
    public final String enhanceStackTraceWithThreadDumpAsJson(DebugCoroutineInfo debugCoroutineInfo) {
        ...
        for (StackTraceElement stackTraceElement : listEnhanceStackTraceWithThreadDump) {
            ...
            sb.append("\n                }\n                ");
            arrayList.add(kotlin.text.l.j(sb.toString()));
        }
        return androidx.compose.runtime.j.l(new StringBuilder("["), p.I(arrayList, null, null, null, null, 63), ']');
    }
    ...
    public final String hierarchyToString$kotlinx_coroutines_core(Job job) {
        ...
        int iJ = kotlin.collections.c0.j(kotlin.collections.r.p(arrayList, 10));
        if (iJ < 16) {
            iJ = 16;
        }
    ...
    public final <T> kotlin.coroutines.d<T> probeCoroutineCreated$kotlinx_coroutines_core(kotlin.coroutines.d<? super T> dVar) {
        if (isInstalled$kotlinx_coroutines_debug() && (!(ignoreCoroutinesWithEmptyContext && dVar.getContext() == kotlin.coroutines.k.e) && owner(dVar) == null)) {
    ...
    public final void uninstall$kotlinx_coroutines_core() throws InterruptedException {
        l lVar;
        if (!isInstalled$kotlinx_coroutines_debug()) {
            kotlinx.coroutines.future.a.u("Agent was not installed");
            return;
        }
