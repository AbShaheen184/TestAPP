...
final /* synthetic */ class FlowKt__MergeKt {
    ...
    public static final <T> Flow<T> flattenMerge(Flow<? extends Flow<? extends T>> flow, int i) {
        if (i <= 0) {
            kotlinx.coroutines.future.a.s(androidx.privacysandbox.ads.adservices.java.internal.a.n(i, "Expected positive concurrency level, but had "));
            return null;
        }
        if (i == 1) {
    ...
    public static final <T> Flow<T> merge(Flow<? extends T>... flowArr) {
        flowArr.getClass();
        return FlowKt.merge(flowArr.length == 0 ? w.e : new n(flowArr, 0));
    }
