...
public final class DispatchedContinuationKt {
    ...
    public static final void safeDispatch(CoroutineDispatcher coroutineDispatcher, j jVar, Runnable runnable) {
        try {
            coroutineDispatcher.mo86dispatch(jVar, runnable);
        } catch (Throwable th) {
            throw new DispatchException(th, coroutineDispatcher, jVar);
        }
    }

    public static final boolean safeIsDispatchNeeded(CoroutineDispatcher coroutineDispatcher, j jVar) throws DispatchException {
        try {
            return coroutineDispatcher.isDispatchNeeded(jVar);
        } catch (Throwable th) {
            throw new DispatchException(th, coroutineDispatcher, jVar);
        }
    }
