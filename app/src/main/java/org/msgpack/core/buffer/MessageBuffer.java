...
public class MessageBuffer {
    ...
    public MessageBuffer(ByteBuffer byteBuffer) {
        if (!byteBuffer.isDirect()) {
            if (!byteBuffer.hasArray()) {
                kotlinx.coroutines.future.a.q("Only the array-backed ByteBuffer or DirectBuffer is supported");
                throw null;
            }
    ...
    public static MessageBuffer allocate(int i) {
        ...
        kotlinx.coroutines.future.a.q("size must not be negative");
        return null;
    }
    ...
    public static void releaseBuffer(MessageBuffer messageBuffer) {
        ...
        try {
            ...
            kotlinx.coroutines.future.a.l(th);
        }
    }
    ...
    public ByteBuffer sliceAsByteBuffer(int i, int i2) {
        ...
        if (constructor == null) {
            kotlinx.coroutines.future.a.u("Can't create a new DirectByteBuffer. In JDK17+, two JVM options needs to be set: --add-opens=java.base/java.nio=ALL-UNNAMED and --add-opens=java.base/sun.nio.ch=ALL-UNNAMED");
            return null;
        }
        try {
            int iE = g.e(d.f);
