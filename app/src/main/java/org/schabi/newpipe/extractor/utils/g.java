...
public abstract class g {
    ...
    public static void a(Pattern pattern, String str) throws org.schabi.newpipe.extractor.exceptions.f {
        if (h(str)) {
            kotlinx.coroutines.future.a.q("Url can't be null or empty");
        } else {
    ...
    public static String b(String str) {
        try {
            URL urlM = m(str);
            return (urlM.getHost().contains("google") && urlM.getPath().equals("/url")) ? URLDecoder.decode(b.i("&url=([^&]+)(?:&|$)", 1, str), StandardCharsets.UTF_8) : str;
    ...
    public static String c(String str) throws org.schabi.newpipe.extractor.exceptions.f {
        try {
            ...
            throw new org.schabi.newpipe.extractor.exceptions.f(j.i("Malformed url: ", str), e);
        }
    }
    ...
    public static String e(String str, String[] strArr) throws e {
        for (Pattern pattern : (Pattern[]) Arrays.stream(strArr).filter(new x(11)).map(new z(17)).toArray(new f())) {
            try {
                String strJ = b.j(pattern, str, 1);
                if (strJ != null) {
                    return strJ;
                }
            } catch (e unused) {
        ...
        throw new e("No regex matched the input on group 1");
    }
    ...
    public static long k(String str) {
        ...
        double d;
        try {
            strI = b.i("[\\d]+([\\.,][\\d]+)?([KMBkmb])+", 2, str);
        } catch (org.schabi.newpipe.extractor.exceptions.f unused) {
            strI = "";
        }
        double d2 = Double.parseDouble(b.i("([\\d]+([\\.,][\\d]+)?)", 1, str).replace(",", "."));
